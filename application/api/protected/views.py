...
@protected.route("/pricing", methods=["GET", "POST"])
@login_required
def pricing_internal():
    return render_template(
        "protected/pricing_internal.html",
        pretty_name=current_app.config["APP_PRETTY_NAME"],
        is_internal=True,
    )
...
def create_checkout_session():
    ...
    if price_type == "price_id_1":
        settings_obj = SettingsSql.query.filter_by(name="STRIPE_PRICE_ID_1").first()
    ...
    try:
        if current_user.customer_id:
            ...
            logger.debug(
                f"Creating checkout session for existing customer: {current_user.customer_id}"
            )
            ...
            # New customer
            logger.debug("Creating checkout session for new customer.")
            checkout_session = stripe.checkout.Session.create(
                line_items=line_items,
        ...
        logger.critical("There was an error with the checkout session creation.")
